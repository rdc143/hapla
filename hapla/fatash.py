...
def main(args, deaf):
    ...
    assert args.alpha_num > 0, "Please select a valid number of alpha values"
    ...
    for z in np.arange(F):  # Loop through files
        ...
        if F > 1:
            ...
            w_cnt += W_chr
            f_out = f"{args.out}.{args.prefix}{z + 1}"
        else:
        ...
        if not args.genome_wide:
            ...
            for it in range(args.admix_iter):
                if B_bat > 1:  # Quasi-Newton mini-batch updates
                    ...
                    for b in np.arange(B_bat):
                        ...
                        functions.laiBatch(
                            Z_chr, P_chr, Q_chr, Q_tmp, Q1, Q2, c_chr, s_bat
                        )
                    functions.laiQuasi(Z_chr, P_chr, Q_chr, Q_tmp, Q1, Q2, c_chr)
                else:
                    functions.laiQuasi(Z_chr, P_chr, Q_chr, Q_tmp, Q1, Q2, c_chr)
                ...
                if ((it + 1) % args.admix_check) == 0:
                    ...
                    if B_bat > 1:
                        if L_cur < (L_bat + args.admix_tole):
                            ...
                            L_pre = L_cur
                            functions.laiQuasi(
                                Z_chr, P_chr, Q_chr, Q_tmp, Q1, Q2, c_chr
                            )
                        else:
                            L_bat = L_cur
            ...
            Q_log = np.log(Q_chr)
            print(
                f"\rEstimating file-specific ancestry proportions.\t\t({time() - ts:.1f}s)"
            )
        ...
        if args.viterbi:  # Compute Viterbi decoding
            D_ind = np.zeros((N, B_chr), dtype=np.uint8)
            if args.alpha is None:
                alpha = np.logspace(-args.alpha_max, -args.alpha_min, args.alpha_num)
                print(f"with majority voting across {len(alpha)} alphas.")
                D_bag = np.zeros((len(alpha), N, B_chr), dtype=np.uint8)
                for a in range(len(alpha)):
                    fatash_cy.viterbi(
                        E_chr, D_bag[a], Q_chr, Q_log, alpha[a], args.simple
                    )
                fatash_cy.voting(D_bag, D_ind, K)
                del D_bag
            else:
                print("with fixed alpha.")
                fatash_cy.viterbi(E_chr, D_ind, Q_chr, Q_log, args.alpha, args.simple)
        else:  # Compute posterior decoding
            L_ind = np.zeros((N, B_chr, K))
            if args.alpha is None:
                alpha = np.logspace(-args.alpha_max, -args.alpha_min, args.alpha_num)
                print(f"with majority voting across {len(alpha)} alphas.")
                D_ind = np.zeros((N, B_chr), dtype=np.uint8)
                ...
                if args.save_posteriors:
                    L_bag = np.zeros((len(alpha), N, B_chr))
                for a in range(len(alpha)):
                    fatash_cy.fwdbwd(E_chr, L_ind, Q_chr, Q_log, alpha[a], args.simple)
        ...
        print(f"Saved {mode} decoding path as {f_out}.path")
        if F > 1:
