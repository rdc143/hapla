...
def main(args):
	...
	for c in np.arange(n_chr):
		...
		if not args.viterbi:
			...
			V = np.zeros((W, K)) # Viterbi matrix
			B = np.zeros((W, K), dtype=np.int32) # Backpointer matrix
			p = np.zeros((n, W), dtype=np.int32) # Optimal viterbi path
		...
		for i in range(n):
			...
			if not args.viterbi:
				...
				fatash_cy.calcViterbi(E, V, Q, T, B, p, N, i)
		print(".")
		...
		if n_chr == 1:
			...
			if not args.viterbi:
				...
				np.savetxt(f"{args.out}.chr{c+1}.path", p, fmt="%i")
				print(f"Saved most probable path as {args.out}.path")
			if args.optim:
